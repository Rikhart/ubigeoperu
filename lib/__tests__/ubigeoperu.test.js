...
describe('ubigeoperu', () => {
  ...
  it('Region por codigo', () => {
    var list=ubigeo.getRegions('150121')
    console.log(list,"regiron")
    if(list.length)
      assert(true, 'Existe un listado de distritos'+JSON.stringify(list[0]));
  });

  it('Provincias por codigo', () => {
    var list=ubigeo.getProvinces('150121')
    console.log(list,"provincia")
    if(list.length)
      assert(true, 'Existe un listado de distritos'+JSON.stringify(list[0]));
  });

  it('Distrito por codigo', () => {
    var list=ubigeo.getDistricts('150121')
    console.log(list,"distrito")
    if(list.length)
      assert(true, 'Existe un listado de distritos'+JSON.stringify(list[0]));
  });
